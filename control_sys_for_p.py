...
def count_up():
	if request.method == 'POST':
		...
		try: 
			...
			if 'round_score' in result :
				if int(result['round_score'][0]) >= 0 and int(result['round_score'][0]) <= 180:
					...
					#Result
					if len(game_data['game_log']) == 8:
						result_data = {}
						result_data['game_log'] = count_01_cul.get_game_log()
						result_data['stats'] = count_01_cul.get_stats()		
						return render_template('countup_result.html',title='countup',game_data = result_data)
		...
		return render_template('countup_game.html',title='countup',game_data=game_data)

	elif request.method == 'GET':
		return render_template('countup_game.html',title='countup')
